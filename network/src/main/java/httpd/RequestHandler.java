	...
	public void run() {
		try {
			...
			if ("GET".equals(tokens[0])) {
				...
				// methods : POST, PUSH, DELETE, HEAD, CONNECT
				// SimpleHTttpServer 에서는 무시(400 Bad Resquest)
				// 과제
...
//			outputStream.write("<h1>이 페이지가 잘 보이면 실습과제 SimpleHttpServer를 시작할 준비가 된 것입니다.</h1>".getBytes("UTF-8"));
	...
	private void reponseStaticResource(OutputStream outputStream, String url, String protocol) throws IOException {
		...
		String contentsType = Files.probeContentType(file.toPath());

		// 응답
		outputStream.write((protocol+" 200 OK\r\n").getBytes("UTF-8"));
		outputStream.write(("Content-Type:" + contentsType + "; charset=utf-8\r\n").getBytes("UTF-8"));
		outputStream.write("\r\n".getBytes());
//		outputStream.write("<h1>이 페이지가 잘 보이면 실습과제 SimpleHttpServer를 시작할 준비가 된 것입니다.</h1>".getBytes("UTF-8"));
